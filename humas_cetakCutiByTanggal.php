...
if (empty($nipInput)) {
    $cetakFilterPresensiByTahunAndBulan = query("SELECT * FROM " . 'tb_cuti' . " WHERE " . "tanggal_cuti >= " . "'$dariTanggal'" . ' AND ' . "tanggal_kembali <= " . "'$sampaiTanggal'");
} else {
    $cetakFilterPresensiByTahunAndBulan = query("SELECT * FROM " . 'tb_cuti' . " WHERE " . "tanggal_cuti >= " . "'$dariTanggal'" . ' AND ' . "tanggal_kembali <= " . "'$sampaiTanggal'" . ' AND ' . "nip = " . "'$nipInput'");
}
...
<head>
    ...
    <title>Cetak Rekap Pengajuan Cuti ';
...
if (empty($nipInput)) {
    $html .= '<h3 style="text-align: center;">Rekapitulasi Cuti dari : ' . $dariTanggal . ' sampai ' . $sampaiTanggal . '</h3>';
} else {
    $html .= '<h3 style="text-align: center;">Rekapitulasi Cuti : <br> [' . $row['nip'] . ' - ' . $row['nama'] . '] <br> <br> ' . $dariTanggal . ' ~ ' . $sampaiTanggal . '</h3>';
}
...
$html .= '<table style="width: 100%; border: 1px solid black;" cellspacing="0" cellpadding="5">
        <tr style="background-color: #BAD7E9;">
            ...
            <td style="border: 1px solid black;">Durasi</td>
